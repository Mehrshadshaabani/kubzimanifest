...
func (s *Store) ListAllServiceOrders(ctx context.Context) ([]ServiceOrderWithUser, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT o.id, o.user_id, o.service_id, o.package_id, o.service_name, o.package_name, o.price_usd, o.contact_name, o.contact_email, o.project_notes, o.status, o.created_at, o.updated_at, u.email
	...
	for rows.Next() {
		...
		if err := rows.Scan(&o.ID, &o.UserID, &o.ServiceID, &o.PackageID, &o.ServiceName, &o.PackageName, &o.PriceUSD, &o.ContactName, &o.ContactEmail, &o.ProjectNotes, &o.Status, &o.CreatedAt, &o.UpdatedAt, &o.UserEmail); err != nil {
			return nil, fmt.Errorf("store: scanning service order: %w", err)
		}
		orders = append(orders, o)
	}
